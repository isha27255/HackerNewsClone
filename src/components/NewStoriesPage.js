...
class NewStoriesPage extends React.Component {
  ...
  render() {
    ...
    {currentItems.map(story => (
        <div key={story.objectID} className="bg-white p-6 rounded-lg shadow-lg border-t-2 border-solid border-indigo-600 p-8 mt-6">
        <a className="text-2xl font-bold mb-2 text-gray-800" href={story.url}>{story.title} ( {story.url!=null? story.url.split("/")[2] : 'none' } )</a>
        <p className="text-gray-500">{story.points} {story.points>1 ? 'points' : 'point'} by {story.author}  | hide | past | discuss </p>
